...
RUN cd OpenClaw-RL && git clone -b v1.2.1 https://github.com/deepseek-ai/DeepEP.git && cd DeepEP &&  sed -i '44i\    disable_nvshmem=True' setup.py  && DISABLE_SM90_FEATURES=1 DISABLE_AGGRESSIVE_PTX_INSTRS=1 /my_workspace/.venv/bin/pip install . --no-build-isolation
